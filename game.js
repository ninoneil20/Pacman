// game.js — Playable web Pac-Man (grid-based) with walls + powerups + one ghost
(() => {
  // Canvas + context
  ...
  // Game state / settings
  let TILE = parseInt(gridSizeSelect.value, 10); // pixels per tile
  ...
  let tickDelay = parseInt(speedInput.value, 10); // ms per step
  let running = false;
  let score = 0;
  ...
  function step() {
    ...
    if (player.c === ghost.c && player.r === ghost.r) {
      if (ghost.vulnerable) {
        ...
        // CHANGE TERRAIN (new grid size & new maze)
  ...
  function render() {
    ...
    ctx.beginPath();
    ctx.moveTo(px, py);
    ctx.arc(px, py, TILE * 0.45, start, end, false);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.arc(gx, gy - TILE * 0.12, TILE * 0.36, Math.PI, 0);
    ctx.fillStyle = ghost.vulnerable ? '#66ffff' : ghostColor;
    ctx.fill();

    ctx.beginPath();
    ctx.rect(gx - TILE * 0.36, gy - TILE * 0.12, TILE * 0.72, TILE * 0.6);
    ctx.fill();
